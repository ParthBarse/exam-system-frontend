...
export default function Discount() {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        axios.post('https://mcfapis.bnbdevelopers.in/addDiscountCodes', reqData).then(e => fetchData()).then(e => setReqData({ discount_amount: '', discount_code: '' })).then(e => toast.success("Discount Added Successfully")).catch(e => console.log(e));
    }
                                            ...
                                            <form className="  w-full rounded px-8 pt-6 pb-8 mb-4">
                                                <div className="grid grid-cols-2 gap-4">
                                                    ...
                                                    <div className="mb-4">
                                                        ...
                                                        <input id="discount_code" name='discount_code' value={reqData.discount_code} type="text" className="w-full px-3 py-2 border rounded shadow appearance-none" placeholder="Enter code" onChange={handleChange} required />
                                                    </div>
                                                    <div className="mb-4">
                                                        <label htmlFor="Discount Amount" className="block text-sm font-medium text-gray-600">Discount Amount</label>
                                                        <input id="Discount Amount" name='discount_amount' value={reqData.discount_amount} type="text" className="w-full px-3 py-2 border rounded shadow appearance-none" placeholder="Enter amount" onChange={handleChange} required />
                                                    </div>

                                                </div>
                                                <button onClick={handleSubmit} className="text-md" style={{ padding: "5px 10px", background: "#007BFF", color: "white", border: "none", borderRadius: "5px", marginRight: "10px" }}>
                                                    Add
                                                </button>
                                            </form>
                                        </div>
                                                    ...
                                                    <tbody className="text-sm font-medium divide-y divide-slate-100 dark:divide-slate-700">
                                                        {discountData.map((item, index) => (
                                                            <tr key={index}>
                                                                ...
                                                                <td className="p-2">
                                                                    <div className="text-center">{item.discount_code}</div>
                                                                </td>
                                                                <td className="p-2">
